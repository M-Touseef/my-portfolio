              ...
              <div className="prose dark:prose-invert max-w-3xl">
                ...
                <div className="grid grid-cols-2 gap-6 mt-12">
                  {[
                    ...
                    <motion.div
                      key={index}
                      className="p-8 bg-white dark:bg-gray-800 rounded-2xl shadow-sm hover:shadow-lg transition-shadow border border-gray-100 dark:border-gray-700"
                      whileHover={{ y: -5 }}
                    >
                      <motion.div
                        className="text-indigo-600 dark:text-indigo-400 text-3xl mb-4 inline-block p-3 rounded-full bg-indigo-50 dark:bg-gray-900"
                        animate={{ rotate: [0, 10, -10, 0] }}
                        transition={{ duration: 2, repeat: Infinity }}
                      >
                        {card.icon}
                      </motion.div>
                      <h4 className="text-xl font-semibold mb-4 text-gray-900 dark:text-white">{card.title}</h4>
            ...
            >
              <div className="relative pl-8">
                ...
                {timelineItems.map((item, index) => (
                  ...
                  >
                    {/* Timeline marker without connecting line */}
                    <motion.div
                      className="absolute left-0 top-1 -translate-x-1/2 w-5 h-5 bg-indigo-600 rounded-full flex items-center justify-center shadow-lg z-10"
                      initial={{ scale: 0 }}
                      animate={{ scale: 1 }}
                    ...
                    <div className="space-y-2 ml-10 pt-2">
                      <motion.div
                        className="p-6 bg-white dark:bg-gray-800 rounded-xl shadow-xs hover:shadow-md transition-shadow border border-gray-100 dark:border-gray-700"
                        whileHover={{ x: 10 }}
                      >
                        <h4 className="text-xl font-semibold dark:text-white mb-3">
                          {item.title}
                        </h4>
                        <motion.p
